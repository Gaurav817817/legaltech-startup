...
export default async function AdminPage({
  ...
  searchParams: Promise<{ loc?: string | string[]; area?: string | string[]; exp?: string | string[] }>
}) {
  ...
  const locs  = toArray(sp.loc)
  const areas = toArray(sp.area)
  const exps  = toArray(sp.exp)

  // Unfiltered totals for stats strip
  ...
  const { data: locationRows } = await supabase
    .from('lawyer_profiles')
    .select('location')
  const locations = [...new Set((locationRows ?? []).map(r => r.location).filter(Boolean))].sort() as string[]
  ...
  if (locs.length > 0)  query = query.in('location', locs)
  if (areas.length > 0) query = query.or(areas.map(a => `practice_areas.cs.{"${a}"}`).join(','))
  if (exps.length > 0)  query = query.in('years_experience', exps)
  ...
  const isFiltered = locs.length > 0 || areas.length > 0 || exps.length > 0
    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      {isFiltered && (
        <p className="text-sm text-blue-700 font-medium mb-4">
          {lawyers?.length ?? 0} lawyer{lawyers?.length === 1 ? '' : 's'} match your filters
        </p>
      )}
