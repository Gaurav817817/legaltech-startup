...
export default function IntakeChatPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="bg-blue-900 text-white py-6 px-4 shadow-md text-center shrink-0">
        <Scale className="w-8 h-8 mx-auto mb-2 text-blue-200" />
        ...
        {messages.map(m => (
          <div key={m.id} className="flex flex-col gap-2">
            <div className={`flex items-start gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${m.role === 'user' ? 'bg-blue-600' : 'bg-gray-200'}`}>
                          ...
                          {result.results && result.results.length > 0 ? (
                            result.results.map((lawyer: any) => (
                              <div key={lawyer.id} className="bg-white border border-gray-200 rounded-lg p-3 flex items-center gap-4 hover:shadow-md transition-shadow">
                                ...
                                <Link href={`/lawyers/${lawyer.id}`} className="bg-blue-50 text-blue-700 p-2 rounded-full hover:bg-blue-100 transition-colors">
                            ...
                            <p className="text-gray-500 text-sm italic">
                              We couldn't find a direct match right now, but our human team can help! Please visit our Contact page.
                            </p>
                          )}
                        </div>
                      </div>
        ...
        <div className="max-w-3xl mx-auto">
          <form onSubmit={(e) => {
            ...
            <button 
              ...
              className="bg-blue-600 text-white rounded-full w-12 h-12 flex items-center justify-center shrink-0 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-md transition-colors"
            >
